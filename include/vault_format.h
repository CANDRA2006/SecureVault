 ...
 *   [8..11]  KDF iterations (uint32_t LE)
 *   [12..43] Salt (32 bytes, CSPRNG)
 *   [44..55] Nonce/IV (12 bytes, CSPRNG)
 *   [56..59] Metadata length in bytes (uint32_t LE, 0 = no metadata)
 ...
 *
 * The GCM AAD covers the full fixed header (bytes [0..59]) so any
 * tampering with any header field—including metadata_len—is detected.
 */
...
// Input safety limits
constexpr size_t   MAX_FILE_SIZE      = 1ULL * 1024 * 1024 * 1024; // 1 GB
constexpr size_t   MAX_PASSWORD_LEN   = 1024;                       // bytes
